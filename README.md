...
Definirea explicită a lui X și y; descrierea claselor. 
    -Incarcare X si y la load_data
    -descrierea claselor - distributia lor - show_class_distribution
Raportarea dimensiunilor datasetului și a tipurilor de variabile. 
    -Realizat prin get_dim_types si dim_types
Distribuția claselor (count + %), cu concluzie privind dezechilibrul.
    - @DE IMPLEMENTAT 
## 3. Verificarea datelor înainte de antrenare
Missing values (count + %), tipurile de coloane (numerice / categoriale, dacă există) și câteva informații de bază despre date (de exemplu: intervale de valori, minime / maxime, valori neobișnuite observabile).
    - Describe + Missing Methods
Identificarea valorilor extreme prin inspecția datelor și explicarea modului în care acestea influențează alegerea preprocesării. Decizii motivate privind imputarea și scalarea.
